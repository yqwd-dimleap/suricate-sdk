...
def test_load_project_skills_agents_md_without_skills_directory(tmp_path):
    """Test that AGENTS.md is loaded even when .suricate/skills doesn't exist.

    This is a regression test for the bug where third-party skill files like
    AGENTS.md were not loaded when the .suricate/skills directory didn't exist.
    """
    # Create AGENTS.md in the work directory (no .suricate/skills)
...
def test_load_project_skills_third_party_with_skills_directory(tmp_path):
    """Test third-party files are loaded alongside skills from .suricate/skills."""
    # Create AGENTS.md in work directory
    (tmp_path / "AGENTS.md").write_text("# AGENTS.md content")

    # Create .suricate/skills directory with a skill
    skills_dir = tmp_path / ".suricate" / "skills"
...
def test_load_project_skills_with_skills_directory(tmp_path):
    """Test load_project_skills loads from .suricate/skills directory."""
    # Create .suricate/skills directory
    skills_dir = tmp_path / ".suricate" / "skills"
    skills_dir.mkdir(parents=True)
...
def test_load_project_skills_agents_directory_precedence(tmp_path):
    ...
    skills_dir = tmp_path / ".suricate" / "skills"
    microagents_dir = tmp_path / ".suricate" / "microagents"
    ...
    (skills_dir / "duplicate.md").write_text(
        "---\nname: duplicate\n---\nFrom .suricate/skills."
    )
    (microagents_dir / "duplicate.md").write_text(
        "---\nname: duplicate\n---\nFrom .suricate/microagents."
    )
...
def test_load_project_skills_merges_agents_and_openhands(tmp_path):
    """Test loading unique skills from .agents/skills and .suricate/skills."""
    agents_dir = tmp_path / ".agents" / "skills"
    openhands_dir = tmp_path / ".suricate" / "skills"
    agents_dir.mkdir(parents=True)
...
def test_load_project_skills_with_microagents_directory(tmp_path):
    """Test load_project_skills loads from .suricate/microagents directory (legacy)."""
    # Create .suricate/microagents directory
    microagents_dir = tmp_path / ".suricate" / "microagents"
    microagents_dir.mkdir(parents=True)
