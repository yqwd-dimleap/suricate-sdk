...
class TestHookConfig:
    ...
    def test_load_discovers_config_in_working_dir(self):
        """Test that load() discovers .suricate/hooks.json in working_dir."""
        hook = {"type": "command", "command": "test-hook.sh"}
        data = {"hooks": {"PreToolUse": [{"matcher": "*", "hooks": [hook]}]}}

        with tempfile.TemporaryDirectory() as tmpdir:
            # Create .suricate/hooks.json in the working directory
            import os

            hooks_dir = os.path.join(tmpdir, ".suricate")
...
def test_issue_2749():
    ...
    https://github.com/yqwd-dimleap/suricate-sdk/issues/2749
    """
    data = {
        "hooks": {
            "Stop": [
