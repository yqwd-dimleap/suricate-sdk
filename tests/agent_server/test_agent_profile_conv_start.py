...
Covers:
- start-from-profile (Suricate + ACP paths)
...
# Skill discovery is patched so Suricate-profile resolves don't hit the network
...
class TestResolveAgentFromProfile:
    ...
    def test_openhands_profile_resolves_to_agent_and_stamps_launched(self):
        ...
        # Suricate profiles always discover the catalog (deny-list needs the
        ...
        # Suricate discovery always runs; its result is threaded through.
    ...
    def test_openhands_profile_forces_llm_stream_true(self):
        """A profile-launched Suricate conversation must guarantee on_token
    ...
    def test_acp_profile_does_not_force_llm_stream(self):
        """The stream-forcing guarantee is Suricate-only: ACP agents emit
    ...
    def test_openhands_default_tools_get_browser_when_usable(self):
        """A default-toolset (tools=None) Suricate profile launch injects the
    ...
    def test_acp_profile_never_gets_browser_injection(self):
        """ACP agents own their tooling — the injection is Suricate-only."""
        from openhands.agent_server.conversation_service import (
            _resolve_agent_from_profile,
        )
    ...
    def test_openhands_default_profile_triggers_discovery(self):
        """An Suricate profile always discovers the skill catalog (the deny-list
...
class TestConversationServiceStartFromProfile:
    ...
    async def test_profile_launch_inherits_the_stored_memory_preference(
        ...
        started from a named Suricate profile or an ACP profile ignored it.
        """
