...
def test_list_open_issues_filters_by_duplicate_candidate_label(monkeypatch):
    ...
    assert module.list_open_issues("Suricate/agent-sdk") == [
        {"number": 1},
        {"number": 3},
    ]
    assert requested_paths == [
        "/repos/Suricate/agent-sdk/issues?state=open&labels=duplicate-candidate&per_page=100&page=1",
        "/repos/Suricate/agent-sdk/issues?state=open&labels=duplicate-candidate&per_page=100&page=2",
        "/repos/Suricate/agent-sdk/issues?state=open&labels=duplicate-candidate&per_page=100&page=3",
    ]
...
def test_list_issue_comments_paginates(monkeypatch):
    ...
    assert module.list_issue_comments("Suricate/agent-sdk", 7) == [
        {"id": 1},
        {"id": 2},
    ]
    assert requested_paths == [
        "/repos/Suricate/agent-sdk/issues/7/comments?per_page=100&page=1",
        "/repos/Suricate/agent-sdk/issues/7/comments?per_page=100&page=2",
        "/repos/Suricate/agent-sdk/issues/7/comments?per_page=100&page=3",
    ]
...
def test_list_comment_reactions_paginates(monkeypatch):
    ...
    assert module.list_comment_reactions("Suricate/agent-sdk", 99) == [
        {"id": 1},
        {"id": 2},
    ]
    assert requested_paths == [
        "/repos/Suricate/agent-sdk/issues/comments/99/reactions?per_page=100&page=1",
        "/repos/Suricate/agent-sdk/issues/comments/99/reactions?per_page=100&page=2",
        "/repos/Suricate/agent-sdk/issues/comments/99/reactions?per_page=100&page=3",
    ]
...
def test_list_helpers_raise_on_non_list_payloads(monkeypatch):
    ...
    with pytest.raises(
        ...
        module.list_open_issues("Suricate/agent-sdk")
    with pytest.raises(
        RuntimeError, match="Expected list response while listing comments"
    ):
        module.list_issue_comments("Suricate/agent-sdk", 7)
    with pytest.raises(
        RuntimeError, match="Expected list response while listing reactions"
    ):
        module.list_comment_reactions("Suricate/agent-sdk", 9)
...
def test_close_issue_propagates_comment_failure(monkeypatch):
    ...
    with pytest.raises(RuntimeError, match="comment failed"):
        module.close_issue_as_duplicate("Suricate/agent-sdk", 123, 45, dry_run=False)

    assert calls == [
        ("POST", "/repos/Suricate/agent-sdk/issues/123/comments"),
    ]
...
def test_dry_run_helpers_skip_api_calls(monkeypatch):
    ...
    assert module.remove_candidate_label("Suricate/agent-sdk", 1, dry_run=True) is True
    assert module.post_veto_note("Suricate/agent-sdk", 1, dry_run=True) is True
    ...
    assert (
        module.close_issue_as_duplicate("Suricate/agent-sdk", 1, 2, dry_run=True)
        is None
    )
...
def test_close_issue_as_duplicate_removes_label_on_success(monkeypatch):
    ...
    assert calls == [
        ("POST", "/repos/Suricate/agent-sdk/issues/123/comments"),
        ("PATCH", "/repos/Suricate/agent-sdk/issues/123"),
        ("REMOVE_LABEL", "Suricate/agent-sdk#123:False"),
    ]
...
def test_keep_open_due_to_newer_comments_removes_candidate_label(monkeypatch):
    ...
    assert calls == [("Suricate/agent-sdk", 123, False)]
...
def test_auto_close_main_honors_author_veto(monkeypatch, capsys):
    ...
    monkeypatch.setattr(
        ...
        lambda: argparse.Namespace(
            repository="Suricate/agent-sdk", close_after_days=3, dry_run=False
        ),
    )
    ...
    assert removed == [("Suricate/agent-sdk", 123, False)]
    assert veto_notes == [("Suricate/agent-sdk", 123, False)]
...
def test_auto_close_main_closes_old_duplicate(monkeypatch, capsys):
    ...
    assert closed == [("Suricate/agent-sdk", 123, 45, False)]
...
def test_auto_close_main_skips_malformed_issue_data(monkeypatch, capsys):
    ...
    summary = json.loads(capsys.readouterr().out)
    assert summary == {"repository": "Suricate/agent-sdk", "results": []}
...
def test_auto_close_main_removes_label_when_newer_comment_exists(monkeypatch, capsys):
    ...
    assert keep_open_calls == [("Suricate/agent-sdk", 123, False)]
...
def test_build_prompt_includes_all_sections():
    ...
    prompt = module.build_prompt(
        "Suricate/agent-sdk",
        {
            ...
            "html_url": "https://github.com/yqwd-dimleap/agent-sdk/issues/123",
        },
    )

    assert "Repository: Suricate/agent-sdk" in prompt
...
def test_build_prompt_handles_missing_fields():
    ...
    prompt = module.build_prompt("Suricate/agent-sdk", {"number": 5})
...
def test_poll_start_task_raises_on_failed_status(monkeypatch):
    ...
    with pytest.raises(RuntimeError, match="Suricate start task failed") as exc:
...
def test_poll_conversation_raises_on_failed_status(monkeypatch):
    ...
    with pytest.raises(
        RuntimeError, match="Suricate conversation ended with failed"
    ) as exc:
        module.poll_conversation(
            "conv-123", poll_interval_seconds=1, max_wait_seconds=10
        )
...
def test_issue_duplicate_main_waits_for_start_task_and_writes_output(
    ...
    assert result["repository"] == "Suricate/agent-sdk"
