...
- This directory (`suricate-tools/openhands/tools/`) contains runtime tool implementations under the `openhands.tools.*` namespace.
...
- Treat `suricate-tools/openhands/tools/__init__.py` as the published surface for `suricate-tools`; `__all__` is considered public API.
