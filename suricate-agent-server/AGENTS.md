...
When adding non-Python files (JS, templates, etc.) loaded at runtime, add them to `suricate-agent-server/openhands/agent_server/agent-server.spec` using `collect_data_files`.
...
schema against the previous `suricate-agent-server` release selected from PyPI,
