...
DEFAULT_PERSISTENCE_DIR = Path("workspace/.suricate")
...
def _get_persistence_dir(config: Config | None = None) -> Path:
    ...
    default = (
        config.conversations_path.parent / ".suricate"
...
def _get_profile_persistence_dir() -> Path:
    ...
    ``OH_PERSISTENCE_DIR`` they fall back to the user's ``~/.suricate``
...
def get_llm_profile_store() -> LLMProfileStore:
    ...
    when set, else ``~/.suricate``. This honors isolated agent-server
