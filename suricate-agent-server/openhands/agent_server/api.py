...
def _default_server_tmux_tmpdir() -> Path:
    return Path(tempfile.gettempdir()) / f"suricate-agent-server-{os.getpid()}"
...
def _cleanup_stale_tmux_sessions() -> None:
    ...
    This function kills all existing sessions on the shared Suricate tmux socket
...
def _create_fastapi_instance(config: Config) -> FastAPI:
    ...
    """
    return FastAPI(
        title="Suricate Agent Server",
        version=version("suricate-agent-server"),
        description=(
            "Suricate Agent Server - REST/WebSocket interface for Suricate AI Agent"
        ),
