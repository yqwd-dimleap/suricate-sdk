...
- ``suricate-tools`` for command splitting and escape doubling
  (``terminal.utils.command``).
- ``suricate-sdk`` for AST-aware security detection
  (the planned ``security.defense_in_depth.*`` analyzers).

Hosting the parser inside ``suricate-sdk`` lets both consumers depend
...
``suricate-tools/openhands/tools/terminal/utils/command.py``: sharing
