...
- This directory (`suricate-sdk/openhands/sdk/`) contains the core Python SDK under the `openhands.sdk.*` namespace.
- Keep new modules within the closest existing subpackage (e.g., `llm/`, `tool/`, `event/`, `agent/`) and follow local naming patterns.
- Add/adjust unit tests under `tests/sdk/` mirroring the SDK path (for example, changes to `suricate-sdk/openhands/sdk/tool/tool.py` should be covered in `tests/sdk/tool/test_tool.py`).
...
Documentation lives in **github.com/yqwd-dimleap/docs** under the `sdk/` folder. When adding features or modifying APIs, you MUST update documentation there.

### Workflow

1. Clone docs repo next to this repository: `git clone https://github.com/yqwd-dimleap/docs.git ../openhands-docs`
2. Create matching branch in both repos
3. Update documentation in `openhands-docs/sdk/` folder
4. **If you are creating a PR to `yqwd-dimleap/suricate-sdk`**, you must also create a corresponding PR to `Suricate/docs` with documentation updates in the `sdk/` folder
...
The `LLM_API_KEY` environment variable may be available in the Suricate development environment and works with the All-Hands LLM proxy (`llm-proxy.eval.all-hands.dev` OR `llm-proxy.app.all-hands.dev`). Please consult the human user for the LLM key if it is not found.
