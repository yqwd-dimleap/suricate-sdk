...
Reads the agent-maintained ``MEMORY.md`` indexes -- ``~/.suricate/memory/``
(user tier) and ``<workspace>/.suricate/memory/`` (project tier) -- into one
...
MEMORY_INDEX_RELPATH: Final[str] = ".suricate/memory/MEMORY.md"
...
def load_memory(
    ...
    if user_index is not None:
        ...
        # ``~/.suricate`` fallback keeps the per-user home path out of the
        # prompt, exactly as MemorySection._user_memory_line does.
        user_header_path = to_posix_path(
            get_user_persistence_dir(Path("~/.suricate")) / "memory" / "MEMORY.md"
        )
