...
class AgentPluginsFormat(PluginFormat):
    ...
    └── dev.suricate/           # our client extension directory (optional)
        ├── commands/
        ├── agents/
        └── hooks/hooks.json
    ```
    ...
    ``dev.suricate/`` directory, and ``entry_command`` under
    ...
    def load_commands(self, plugin_dir: Path) -> list[CommandDefinition]:
        """Load command definitions from ``dev.suricate/commands/``."""
