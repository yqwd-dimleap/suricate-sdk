...
def _load_plugins_from_dir(plugins_dir: Path, skip_dirs: set[Path]) -> list[Plugin]:
    ...
    ``~/.suricate/plugins/installed`` — a child of ``~/.suricate/plugins`` — and
...
def load_user_plugins() -> list[Plugin]:
    ...
    Scans ``~/.agents/plugins`` then ``~/.suricate/plugins`` (earlier wins on a
    name conflict), then appends enabled installed plugins from
    ``~/.suricate/plugins/installed`` (lower precedence than directory plugins).
...
def load_project_plugins(work_dir: str | Path) -> list[Plugin]:
    ...
    Scans ``{root}/.agents/plugins`` and ``{root}/.suricate/plugins`` for the
