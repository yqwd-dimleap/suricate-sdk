...
def get_user_persistence_dir(default: Path | None = None) -> Path:
    ...
    otherwise returns ``default``, or a home-directory fallback when no
    ``default`` is given. ``OH_PERSISTENCE_DIR`` replaces the home-directory
    base; callers append their usual subdirectories (e.g. ``profiles``,
    ``cache/skills``) to the result.

    Home-directory fallback (when neither env nor ``default`` is set):

    1. ``~/.suricate`` if that directory already exists
    2. ``~/.openhands`` if that directory already exists (pre-rename local
       installs — Desktop still keeps canvas state under
       ``~/.openhands/agent-canvas`` and sets ``OH_PERSISTENCE_DIR`` to
       ``~/.openhands``)
    3. ``~/.suricate`` for brand-new installs
    ...
    if default is not None:
        return default
    home = Path.home()
    suricate = home / ".suricate"
    openhands_legacy = home / ".openhands"
    if suricate.is_dir():
        return suricate
    if openhands_legacy.is_dir():
        return openhands_legacy
    return suricate


# Project-local config directory names, preferred order. ``.suricate`` is the
# Suricate brand; ``.openhands`` is kept so existing checkouts keep working.
PROJECT_CONFIG_DIR_NAMES: tuple[str, ...] = (".suricate", ".openhands")


def iter_project_config_dirs(project_dir: Path | str) -> list[Path]:
    """Return existing project config roots under ``project_dir``.

    Prefers ``.suricate`` when both exist so new branding wins for dual trees.
    """
    root = Path(project_dir)
    return [root / name for name in PROJECT_CONFIG_DIR_NAMES if (root / name).exists()]


def resolve_project_config_file(
    project_dir: Path | str, *relative_parts: str
) -> Path | None:
    """Return the first existing ``project_dir/<config>/relative_parts`` path."""
    root = Path(project_dir)
    for name in PROJECT_CONFIG_DIR_NAMES:
        candidate = root.joinpath(name, *relative_parts)
        if candidate.exists():
            return candidate
    return None
...
def is_local_path_source(source: str) -> bool:
    ...
    ``.suricate``), host-native absolute paths, Windows absolute paths, and
