...
- **Built-in agents**: supplied by `suricate-tools`, outside this SDK package.
...
- `suricate-tools/openhands/tools/preset/default.py`: built-in agent discovery and
  registration.
...
2. `{project}/.suricate/agents/*.md`

**User-level:**

3. `~/.agents/agents/*.md`
4. `~/.suricate/agents/*.md`
...
3. Project file-based agents
   - `{project}/.agents/agents/*.md` then `{project}/.suricate/agents/*.md`
4. User file-based agents
   - `~/.agents/agents/*.md` then `~/.suricate/agents/*.md`

Built-ins are discovered and registered separately by `suricate-tools` through
...
1. **Within a level** (`load_project_agents` / `load_user_agents`):
   - `.agents/agents` wins over `.suricate/agents` for the same agent name.
