...
`.agents/agents` or `.suricate/agents` directories at the project or user level.
...
Directory convention (in priority order):
    ...
    ~/.suricate/               # User-level, legacy (lowest file priority)
        agents/
            my-global-agent.md
...
Priority (highest to lowest):
  ...
  4. Project-level `.suricate/agents/*.md`
  5. User-level `~/.agents/agents/*.md`
  6. User-level `~/.suricate/agents/*.md`
"""
...
_FILE_BASED_AGENTS_DIR: Final[list[str]] = [
    ".agents/agents",
    ".suricate/agents",
    ".openhands/agents",  # Pre-rename local checkouts
]
...
def load_project_agents(project_dir: str | Path) -> list[AgentDefinition]:
    ...
    Searches for
        ...
        - project_dir/.suricate/agents (in that order).
...
def _user_agents_dir(relative: str) -> Path:
    ...
    ``.suricate/agents`` goes under the persistence dir, which replaces the
    ``~/.suricate`` base; every other entry stays home-relative.
    """
...
def discover_agents(
    ...
    *,
    ...
    ``suricate-tools`` (not an SDK dependency), so callers wanting them add
