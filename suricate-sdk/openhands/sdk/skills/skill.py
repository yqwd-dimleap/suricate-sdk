...
class Skill(BaseModel):
    ...
    def load(
        ...
        Supports both Suricate-specific frontmatter fields and AgentSkills
    ...
    def _load_legacy_openhands_skill(
        ...
        """Load a skill from a legacy Suricate-format file.
...
def load_skills_from_dir(
    ...
    - Suricate format: skills/*.md files
    - AgentSkills format: skills/skill-name/SKILL.md directories

    Note, legacy repo instructions will not be loaded here.

    Args:
        skill_dir: Path to the skills directory (e.g. .suricate/skills)
...
def load_user_skills() -> list[Skill]:
    ...
    Searches for skills in ~/.agents/skills/, ~/.suricate/skills/, and
    ~/.suricate/microagents/ (legacy). Skills from all directories are merged,
    with earlier entries in USER_SKILLS_DIRS taking precedence for duplicate
    names.

    Also loads enabled installed skills from ~/.suricate/skills/installed/
...
def load_project_skills(work_dir: str | Path) -> list[Skill]:
    ...
    Searches for skills in {work_dir}/.agents/skills/, {work_dir}/.suricate/skills/,
    and {work_dir}/.suricate/microagents/ (legacy).
    ...
    Use .agents/skills for new skills. .suricate/skills is the legacy Suricate
    location, and .suricate/microagents is deprecated.

    Example: If "my-skill" exists in both .agents/skills/ and .suricate/skills/,
    ...
    # search root. This ensures they are loaded even if .suricate/skills doesn't
    # exist.
    ...
    # Load project-specific skills from .agents/skills, .suricate/skills,
    # legacy .openhands/skills, and microagents (priority order; first wins)
    for root in search_roots:
        project_skills_dirs = [
            root / ".agents" / "skills",
            root / ".suricate" / "skills",
            root / ".openhands" / "skills",  # Pre-rename local checkouts
            root / ".suricate" / "microagents",
...
PUBLIC_SKILLS_REPO = "https://github.com/yqwd-dimleap/extensions"
...
def load_public_skills(
    ...
    """Load skills from the public Suricate skills repository.

    This function maintains a local git clone of the public skills registry at
    https://github.com/yqwd-dimleap/extensions. On first run, it clones the repository
    to ~/.suricate/skills-cache/. On subsequent runs within the same process, it
...
def load_available_skills(
    ...
    *,
    ...
    Args:
        ...
        include_public: Load public skills from the Suricate extensions repo.
