"""Example: OpenHandsCloudWorkspace for Suricate Cloud API.

This example demonstrates using OpenHandsCloudWorkspace to provision a sandbox
via Suricate Cloud (app.all-hands.dev) and run an agent conversation.
...
Requirements:
  ...
  - OPENHANDS_CLOUD_API_KEY: API key for Suricate Cloud access
...
logger.info(f"Using Suricate Cloud API: {cloud_api_url}")
...
with OpenHandsCloudWorkspace(
    ...
    result = workspace.execute_command(
        "echo 'Hello from Suricate Cloud sandbox!' && pwd"
    )
