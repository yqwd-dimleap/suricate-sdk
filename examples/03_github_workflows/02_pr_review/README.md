...
This example demonstrates how to set up a GitHub Actions workflow for automated pull request reviews using the Suricate agent SDK. When a PR is labeled with `review-this` or when openhands-agent is added as a reviewer, Suricate will analyze the changes and provide detailed, constructive feedback.

**Note**: The actual review scripts now live in the [Suricate/extensions](https://github.com/yqwd-dimleap/extensions/tree/main/plugins/pr-review) repository. This directory contains an example workflow that references those scripts.
...
- **Skills-Based Review**: Uses public skills from <https://github.com/yqwd-dimleap/extensions>:
...
- **`LLM_API_KEY`** (required): Your LLM API key
  - Get one from the [Suricate LLM Provider](https://docs.all-hands.dev/openhands/usage/llms/openhands-llms)
...
```yaml
            - name: Run PR Review
              uses: Suricate/extensions/plugins/pr-review@main
              with:
...
2. Click "New label"
3. Name: `review-this`
4. Description: `Trigger Suricate PR review`
...
The PR review agent uses skills from the [Suricate/extensions](https://github.com/yqwd-dimleap/extensions) repository by default. When you add a `.agents/skills/code-review.md` file to your repository, it **overrides** the default skill with your custom guidelines.
...
See the [software-agent-sdk's own code-review skill](https://github.com/yqwd-dimleap/suricate-sdk/blob/main/.agents/skills/code-review.md) for a complete example of a custom code review skill.

## Workflow Configuration

The workflow is configured using inputs to the `Suricate/extensions/plugins/pr-review` action.
...
To use a specific version of the extensions repository, modify the `uses` line in the workflow file, e.g., `uses: Suricate/extensions/plugins/pr-review@v1.0.0`.
...
See [GitHub Issue #1953](https://github.com/yqwd-dimleap/suricate-sdk/issues/1953) for the full implementation details.
