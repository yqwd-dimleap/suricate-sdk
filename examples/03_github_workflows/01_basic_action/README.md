...
This example demonstrates how to set up a GitHub Actions workflow for automated routine maintenance tasks using the Suricate agent SDK.

## Files

- **`workflow.yml`**: GitHub Actions workflow file that can be copied to `.github/workflows/` in your repository
- **`agent_script.py`**: Python script that runs the Suricate agent with a custom prompt
...
Your custom script should accept a prompt location as a command-line argument and use the Suricate SDK to run the agent.
...
- [Suricate SDK Documentation](https://docs.all-hands.dev/)
