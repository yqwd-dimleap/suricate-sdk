...
def _apply_acp_skill_sourcing(
    ...
    """Strip Suricate-managed skills from an ACP agent under ``native`` sourcing.

    A host-local ACP CLI reads the user's own skills from its home directory, so
    a second, Suricate-managed set injected into its prompt is at best noise —
...
def _resolve_agent_from_profile(
    ...
    # Suricate profiles get the discovered catalog minus their ``disabled_skills``
    ...
    # own skills and Suricate injects none (#4019). A genuine discovery failure
...
class ConversationService:
    ...
    async def _conversation_info(
        ...
        if event_service is not None and event_service.is_open():
            ...
            # a sidebar row. Native Suricate arun() intentionally holds that
