...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Start an Suricate Cloud conversation that checks a GitHub issue "
            "for duplicates."
        )
    )
    parser.add_argument(
...
def poll_start_task(
    ...
    while time.time() < deadline:
        ...
        if status in {"ERROR", "FAILED"}:
            raise RuntimeError(
                f"Suricate start task failed: {summarize_openhands_item(item)}"
            )
        time.sleep(poll_interval_seconds)
    raise TimeoutError(
...
def poll_conversation(
    ...
    while time.time() < deadline:
        ...
        if execution_status in FAILED_EXECUTION_STATUSES:
            raise RuntimeError(
                "Suricate conversation ended with "
...
def main() -> int:
    ...
    if session_api_key_value and not isinstance(session_api_key_value, str):
        raise RuntimeError(
            "session_api_key had unexpected type in the Suricate conversation: "
        ...
        try:
            ...
            if not session_api_key:
                raise RuntimeError(
                    ...
                    "session_api_key was missing from the Suricate conversation"
                ) from exc
