"""Suricate Cloud workspace implementation using Cloud API."""
...
# Standard exposed URL names from Suricate Cloud
...
class OpenHandsCloudWorkspace(RemoteWorkspace):
    """Remote workspace using Suricate Cloud API.

    This workspace connects to Suricate Cloud (app.all-hands.dev) to provision
    ...
    running inside an Suricate Cloud Runtime sandbox.  Instead of creating or
    ...
    Example:
        ...
        # Running inside an Suricate Cloud Runtime (local agent-server mode)
        workspace = OpenHandsCloudWorkspace(
    ...
    local_agent_server_mode: bool = Field(
        default=False,
        description=(
            "When True, assume the SDK is running inside an Suricate Cloud "
    ...
    cloud_api_url: str = Field(
        description=(
            "Base URL of Suricate Cloud API "
    ...
    cloud_api_key: str = Field(
        description=(
            "API key for authenticating with Suricate Cloud. "
            "Required in all modes — used for get_llms / get_secrets."
        ),
    )
    ...
    def _api_headers(self) -> dict[str, str]:
        ...
        Uses Bearer token authentication as per Suricate Cloud API.
        """
    ...
    def _create_new_sandbox(self) -> None:
        ...
        logger.info("Starting sandbox via Suricate Cloud API...")
    ...
    def pause(self) -> None:
        ...
        Note: Suricate Cloud does not currently support pausing sandboxes.
