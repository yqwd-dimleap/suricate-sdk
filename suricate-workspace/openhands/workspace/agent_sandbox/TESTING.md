...
- **Local / in-cluster LLM:** the default template's `NetworkPolicy` blocks private
  ...
  ```bash
  kubectl patch sandboxtemplate suricate-agent-server --type merge \
...
| `AgentSandboxWorkspace requires the 'agent-sandbox' extra` | `pip install suricate-workspace[agent-sandbox]`. |
